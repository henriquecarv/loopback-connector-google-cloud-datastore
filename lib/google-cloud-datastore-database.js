...
class GoogleCloudDatastore {
   ...
   */
  generateOrderingObject(orderOption) {
  ...
  buildQuery(model, filter) {
    ...
    if (order) {
      ...
      for (const option of order) {
        ...
        query = query.order(property, this.generateOrderingObject(orderOption));
      }
    }
  ...
  addFiltersToQuery(query, filter) {
    ...
    if (isObject) {
      ...
      if (between) {
        return this.addBetweenFilterToQuery(query, key, between);
      }
  ...
  /**
   * Add Between filters to a Query
   * @param {Query} query Datastore Query
   * @param {String} key Property name being filtered
   * @param {Array} between Array with two values for comparison
   */
  addBetweenFilterToQuery(query, key, between) {
    try {
      const [first, second] = between;

      return query.filter(key, '>', first).filter(key, '<', second);
    } catch (error) {
      throw error;
    }
  }
