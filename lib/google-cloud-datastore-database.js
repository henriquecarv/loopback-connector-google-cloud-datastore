...
class GoogleCloudDatastore {
   ...
   */
  async getResultsWithQuery(model, where, limit, fields) {
  ...
  async all(model, filter, options, callback) {
    const {fields, limit, where} = filter;
    ...
    try {
      ...
      if (where && where.id) {
        ...
        result = await this.getResultsWithQuery(model, where, limit, fields);
      } else {
  ...
  async update(model, filter, data, options, callback) {
    try {
      ...
      if (where && where.id) {
        ...
        const entities = await this.getResultsWithQuery(model, where);
