...
class GoogleCloudDatastore {
  ...
  async getResultsWithQuery(model, filter) {
    const {where, order, limit, fields} = filter;
    const query = this.buildQuery(model, where, order, limit, fields);
    const entities = await query.run();
  ...
  async all(model, filter, options, callback) {
    ...
    try {
      ...
      } else if (where || limit || fields || order) {
   ...
   * @param {Object} order The order for expected results
   * @param {Number} limit The number of results expected
   * @param {Object} fields The fields to be returned
   */
  buildQuery(model, where, order, limit, fields) {
    ...
    if (order) {
      ...
      for (let i = 0; i < order.length; i += 1) {
        const option = order[i].split(' ');
        if (option[1] == 'DESC') {
          query.order(option[0], {descending: true});
        } else {
          query.order(option[0]);
        }
      }
    }
